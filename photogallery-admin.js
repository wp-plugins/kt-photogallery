(function ($) {
    $(function () {
        ...
        if (isAlbum || isGallery) {
            ...
            var $Tabs = $('#layoutTabs');
            var $LayoutStandart = $('#layoutStandart');
            var $LayoutStandartForm = $('#layoutStandartForm');
            var $LayoutCustom = $('#layoutCustom');
            var saveData = function (data) {
            ...
            var saveLayoutData = function () {
                var data = {
                    action: 'save_default_template'
                };
                $LayoutStandart.find('.chooser').each(function () {
                    var $chooser = $(this);
                    data[$chooser.attr('id')] = $chooser.children('.active').data('type');
                });
                $($LayoutStandartForm.serializeArray()).each(function () {
            ...
            $Tabs.on('mousedown', 'span', function () {
                var $tab = $(this);
                if (!$tab.hasClass(cssActive)) {
                    $tab.addClass(cssActive).siblings().removeClass(cssActive);
                    $($tab.data('content')).addClass(cssActive).siblings().removeClass(cssActive);
                    if ($tab.attr('data-content') != '#layoutAPI') {
                        saveData({
                            action: 'save_template_type',
                            type: $tab.hasClass('custom') ? 'custom' : 'default'
                        });
                    }
                }
            });
            var toggleDependancies = function () {
                var dependancy, $label;
                $LayoutStandart.find('label').each(function () {
                    $label = $(this);
                    if (!$label.hasClass(cssHidden)) {
                        dependancy = $label.data('dependancy');
                        if (dependancy) {
                            $LayoutStandart.find(dependancy).toggleClass(cssHidden, !$label.children(':checkbox')[0].checked);
                        }
                    }
                });
            };
            var toggleOptions = function ($section, type) {
                $section.find('label').addClass(cssHidden).filter('label.' + type + ', label.all').removeClass(cssHidden);
            };
            $LayoutStandart.find('.chooser').each(function () {
                var $chooser = $(this);
                $chooser.on('mousedown', 'li', function () {
                    var $option = $(this);
                    if (!$option.hasClass(cssActive)) {
                        $option.addClass(cssActive).siblings().removeClass(cssActive);
                        var $section = $chooser.closest('section');
                        toggleOptions($section, $option.data('type'));
                        toggleDependancies();
                        saveLayoutData();
                    }
                });
                var $section = $chooser.closest('section');
                toggleOptions($section, $chooser.children('.' + cssActive).data('type'));
                toggleDependancies($section);
            });
            $LayoutStandart.on('change', ':checkbox', function () {
                toggleDependancies();
                saveLayoutData();
            });
            $LayoutStandart.on('blur', 'input', saveLayoutData);
            $LayoutStandart.on('change', 'select', saveLayoutData);

            $LayoutCustom.on('mousedown', 'h4.toggle', function(){
